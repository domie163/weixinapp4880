			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.shangjianicheng" type="text" placeholder="商家昵称" ></input>
					</view>
				<view class="action">
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.shangjianicheng="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.shangjianicheng){
			...
			async search(){
				...
				if(this.searchForm.shangjianicheng){
					searchForm['shangjianicheng'] = '%' + this.searchForm.shangjianicheng + '%'
				}
                let res = {};
                if(this.userid) {
