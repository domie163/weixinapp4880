	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#E5FFFC","height":"auto"}'>
		<form :style='{"width":"94%","padding":"24rpx","margin":"5% 3%","background":"#fff","display":"block","height":"90%"}' class="app-update-pv">
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.tonggaozhuangtai" v-model="ruleForm.tonggaozhuangtai" placeholder="通告状态"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`tonggaozhuangtai`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`tonggaozhuangtai`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`tonggaozhuangtai`, this.ruleForm);
						}else{
							await this.$api.add(`tonggaozhuangtai`, this.ruleForm);
						}
