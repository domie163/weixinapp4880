...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','baomingzhuangtai');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','baomingzhuangtai');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('baomingzhuangtai', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussbaomingzhuangtai/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
